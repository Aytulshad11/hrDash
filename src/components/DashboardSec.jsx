...
//       </div>
//       <div className="p-6 pt-0 flex-grow bg-slate-100">
//         <br />
    ...
    >
      ...
      <div className="p-6 pt-0 flex-grow  space-y-4">
        {items.map((item, i) => (
          <div
            key={i}
            className="flex items-start justify-between p-4 rounded-lg border bg-gray-200"
          >
            <div className="space-y-1">
              <p className="text-sm font-medium leading-none">{item.title}</p>
              <p className="text-sm text-muted-foreground">{item.timestamp}</p>
      ...
      <button
        className={cn(
          "mt-auto w-full rounded-md px-4 py-2 text-sm font-medium text-red-800",
          variant === "activity"
            ? "bg-secondary text-secondary-foreground hover:bg-secondary/80"
            : "text-primary underline-offset-4 hover:underline"
        )}
